...
const List = () => {
    ...
    const checkNetwork = (network) => {
        ...
        if (chainId === 56 && network === 'ether') {
            ret.isValid = false;
            ret.message = "Please connect with Ethereum network";
        } else if (chainId === 1 && network === 'bsc') {
            ret.isValid = false;
    ...
    const getContract = () => {
        const contractAddress = chainId === 1 ? EthereumContractAddress : BinanceContractAddress
        const contractABI = chainId === 1 ? etherContractAbi : binanceContractAbi
        ...
        if (window.confirm("Are you sure to cancel listing?")) {
            ...
            await contract.methods.removeFromListedPrice(listData.tokenId)
                ...
                .on("receipt", async (receipt) => {
                    ...
                    toast.success("Remove from listed for sell")
                })
                .on("error", (err) => {
