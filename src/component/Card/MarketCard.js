                    ...
                    <p className="mb-0 text-left color-black card-text justify-content-between d-flex">
                        {
                            data.highestOfferPrice !== 0 ? (
                                <span className="color-grey font-very-very-small d-flex align-items-center">
                                    <img src={parseInt(data.highestOfferType) === 1 ? (data.network === 'ether' ? EthImg : BscImg) : GolfImg} alt="" height="14px" />
                                    &nbsp;
                                    {parseInt(data.highestOfferType) === 1 ? Web3.utils.fromWei(Number(data.highestOfferPrice).toString(), 'ether') : data.highestOfferPrice} (Highest Offer)
                                </span>
                            ) : (
                                data.lastSoldPrice !== 0 ? (
                                    <span className="color-grey font-very-very-small d-flex align-items-center">
                                        <img src={parseInt(data.lastSoldCurrency) === 1 ? (data.network === 'ether' ? EthImg : BscImg) : GolfImg} alt="" height="14px" />
                                        &nbsp;
                                        {parseInt(data.lastSoldCurrency) === 1 ? Web3.utils.fromWei(Number(data.lastSoldPrice).toString(), 'ether') : data.lastSoldPrice} (Last Sold)
                                    </span>
                                ) : (
                        ...
                        <span className="color-green text-right d-flex align-items-center">
                            {
                                <img src={(parseInt(data.listType) === 0 || parseInt(data.listType) === 1) ? (data.network === 'bsc' ? BscImg : EthImg) : GolfImg} alt="" height="16px" />
                            }&nbsp;
                            {data.listPrice === 0 ?
